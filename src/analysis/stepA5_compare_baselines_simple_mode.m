...
%       with tau_max = 1 and sigma(k) ∈ {+1, 0, -1}. Fit (delta_plus, delta_minus)
...
% =====================================================================
% Fallback: evaluate wRMSE for candidate theta_hat_* (SIMPLE model)
% =====================================================================

function wRMSE = local_eval_model_wrmse_only(theta, participants, dt, weights, cfg) %#ok<INUSD>
    % Compute overall weighted RMSE for a candidate theta by simulating SIMPLE
    % mode and comparing predictions to aligned measurements.
    r_all = zeros(0,1);
    w_all = zeros(0,1);
